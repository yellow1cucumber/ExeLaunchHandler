...
bool shouldShowHelp(const int argc, char* argv[]) {
...
bool isUIModeEnabled(const int argc, char* argv[]) {
    auto options = createOptions();
    const auto result = options.parse(argc, argv);
    return result["ui"].as<bool>();
}

Configuration::AppConfig initApplication(const std::string& configPath) {
...
bool runPipeline(const std::string& configPath) {
...
bool launchProcess(const Configuration::AppConfig& config) {
